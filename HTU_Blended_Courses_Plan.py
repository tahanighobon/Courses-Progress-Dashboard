...
TLC_SHEETS = [ "https://docs.google.com/spreadsheets/d/1y7mPQzNxkGXMKqBVEk1X_icALvotanOkL3HL885sMAY/gviz/tq?tqx=out:csv&gid=0", "https://docs.google.com/spreadsheets/d/1Ksh_5KUAyuE_H_rJkf0vDRvSKJxvyt2sYSzDgLwR5Nw/gviz/tq?tqx=out:csv&gid=0", "https://docs.google.com/spreadsheets/d/1bRHPX7vvU49A0Q_WzaKhNwhjqS9ketpEJKU64GLSIuM/gviz/tq?tqx=out:csv&gid=0", "https://docs.google.com/spreadsheets/d/1B5o0uBdFrR-pGT9dxStLorAgWx3XUYyN6I-yiBZlMcc/gviz/tq?tqx=out:csv&gid=0", ] 
...
def is_filled(x) -> bool: if x is None or (isinstance(x, float) and pd.isna(x)): return False s = str(x).strip() if s == "": return False if s.lower() in {"nan", "none", "null"}: return False return True 
...
def norm_bool(x) -> bool: if isinstance(x, bool): return x if x is None or (isinstance(x, float) and pd.isna(x)): return False s = str(x).strip().lower() return s in {"true", "yes", "1", "✓", "✔", "✅", "done"} 

def render_donut_chart(percent: float, key: str, size_px=170): pct = 0.0 if pd.isna(percent) else float(percent) pct = max(0.0, min(100.0, pct)) 
...
def instructor_mentioned_in_cell(cell_value, instructor_name: str) -> bool: if not is_filled(cell_value): return False txt = clean_name(str(cell_value)).lower() inst = clean_name(instructor_name).lower() return inst in txt 
...
@st.cache_data def load_data(): df = pd.read_csv(DATA_URL) df.columns = df.columns.astype(str).str.strip() 
...
@st.cache_data def load_tlc_sessions(): frames = [] for url in TLC_SHEETS: try: d = pd.read_csv(url) except Exception: continue 
...
st.sidebar.image("htu_logo.png", use_container_width=True) st.sidebar.markdown(" 

 

", unsafe_allow_html=True) 

page = st.sidebar.radio( "Go to", [ "🏠 Home", "🏫 Instructors", "🌱 Spring 2024/2025", "🍂 Fall 2025/2026", "🌸 Spring 2025/2026", ] ) 

if page in ["🌱 Spring 2024/2025", "🍂 Fall 2025/2026", "🌸 Spring 2025/2026"]: view = st.sidebar.radio("View", ["Overview", "Schools"]) 
...
HTU Digital Twin by 2028 Progress 
...
if page == "🏠 Home": summary = [ {"school": "SCI", "total": 37, "ready": 8}, {"school": "SET", "total": 69, "ready": 9}, {"school": "SBEE", "total": 30, "ready": 2}, {"school": "SSBS", "total": 32, "ready": 12}, ] for s in summary: s["percent"] = 0 if s["total"] == 0 else round(s["ready"] / s["total"] * 100, 1) 
...
if page == "🏫 Instructors": st.subheader("Instructors") 
...
if page == "🌱 Spring 2024/2025": render_semester_page(df_all, "Spring 2024/2025", view, "spring2425") 

if page == "🍂 Fall 2025/2026": render_semester_page(df_all, "Fall 2025/2026", view, "fall2526") 

if page == "🌸 Spring 2025/2026": render_semester_page(df_all, "Spring 2025/2026", view, "spring2526") 
