...
st.set_page_config(page_title="HTU Digital Twin by 2028 Progress", layout="wide")
...
def load_data():
    ...
    for possible in possible_course_cols:
    ...
    df["Progress %"] = df.apply(
        lambda r: compute_progress_percent(r, df.columns.tolist()),
        axis=1,
    )
...
def render_semester_page(df_all: pd.DataFrame, semester_label: str, view: str, key_prefix: str):
    ...
    if view == "Overview":
        ...
        schools = [s for s in df["School"].dropna().unique() if clean_text_value(s) != ""]
        ...
        departments = [d for d in d1["Department"].dropna().unique() if clean_text_value(d) != ""]
        ...
        if dept == "— Select Department —":
            ...
            school_table = school_table.rename(
                columns={
        ...
        courses = [c for c in d2["Course \\ pathway"].dropna().unique() if clean_text_value(c) != ""]
        ...
        course_options = ["— Select Course —"] + courses
...
if os.path.exists("htu_logo.png"):
...
st.markdown("<div class='main-title'>HTU</div>", unsafe_allow_html=True)
st.markdown("<div class='sub-title'>HTU Digital Twin by 2028 Progress</div>", unsafe_allow_html=True)
...
if page == "🏠 Home":
    ...
    school_summary = (
        df_all.groupby("School", dropna=False)
        .agg(
            total=("Course \\ pathway", "count"),
            ready=("Progress %", lambda s: int((s >= 99.9).sum())),
            avg_progress=("Progress %", "mean"),
        )
        .reset_index()
    )

    school_summary["School"] = school_summary["School"].apply(clean_text_value)
    school_summary = school_summary[school_summary["School"] != ""].copy()
    school_summary = school_summary.sort_values("School").reset_index(drop=True)

    total_courses = int(school_summary["total"].sum())
    total_ready = int(school_summary["ready"].sum())
    ...
    if not school_summary.empty:
        cols = st.columns(len(school_summary))
        for i, (_, row) in enumerate(school_summary.iterrows()):
            school = row["School"]
            total = int(row["total"])
            ready = int(row["ready"])
            ...
            with cols[i]:
                ...
                st.caption(f"Progress: {percent:.1f}%")
    else:
...
# ==========================
    ...
    school_options = [s for s in df_all["School"].dropna().unique() if clean_text_value(s) != ""]
    school_options = sorted(school_options)
    ...
    if len(school_options) == 0:
        ...
        department_options = [d for d in df_s["Department"].dropna().unique() if clean_text_value(d) != ""]
            ...
            if len(all_instructors) == 0:
                ...
                if df_i.shape[0] == 0:
                        ...
                        rows.append(
                            {
                                ...
                                "Total Progress": (
                                    "" if pd.isna(r.get("Progress %", np.nan))
                                    else f"{float(r.get('Progress %')):.1f}%"
                                ),
                    ...
                    for _, r in df_i.iterrows():
                        ...
                        worked_any = instructor_mentioned_in_cell(r.get("Detailed Outline", ""), instructor)
                        if not worked_any:
                            for i in range(1, 16):
                    ...
                    if tlc_match.shape[0] == 0 and df_tlc.shape[0] > 0:
                        tlc_match = df_tlc[
                            df_tlc["__name_key__"].astype(str).str.contains(re.escape(instructor_key), na=False)
                        ].copy()
                    ...
                    if tlc_match.shape[0] == 0:
                        ...
                        for c in session_cols:
                            ...
                            if done:
                                completed += 1
                            session_rows.append(
                                {"Session": c, "Completion": "✅" if done else "❌"}
                            )
...
st.markdown("<br><br>", unsafe_allow_html=True)
st.markdown(
    "<div style='text-align:center; color:#777777;'>Made By: The D. Learn Center at HTU</div>",
    unsafe_allow_html=True,
)
